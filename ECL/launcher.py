...
class EuoraCraftLauncher:
    ...
    def _init_core_resources(self) -> None:
    ...
    def _ensure_game_paths(self) -> None:
        logger.info("检查游戏目录...")
    ...
    async def _preload_version_list(self) -> None:
        ...
        if not versions:
            logger.warning("版本列表预加载失败")
            return

        self.state.get_games._cached_versions = versions
        version_count = len(versions.get("All", []))
    ...
    async def init_launcher(self):
        ...
        try:
            ...
            if not state_result.get("config"):
                return False
            ...
            self._ensure_game_paths()
            self._setup_debug_mode()
            _ = asyncio.create_task(self._preload_version_list())  # noqa: RUF006
