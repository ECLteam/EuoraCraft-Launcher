...
class PluginFramework:
    def __init__(
        ...
        self._load_deps_meta()

    def _load_deps_meta(self) -> None:
    ...
    def _load_plugin_config(self, plugin_name: str) -> dict[str, Any]:
        path = self._get_plugin_config_path(plugin_name)
        if path.exists():
            try:
    ...
    def _save_plugin_config(self, plugin_name: str, values: dict[str, Any]) -> None:
        try:
            self._get_plugin_config_path(plugin_name).write_text(
                json.dumps(values, ensure_ascii=False, indent=2), "utf-8"
            )
        except OSError as e:
    ...
    def _load_plugin_internal(self, plugin_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                self.enable_plugin_internal(plugin_name)
            except (RuntimeError, OSError, ValueError, TypeError) as e:
                logger.warning(f"插件 {plugin_name} 加载成功但启用失败: {e}")

            logger.info(f"插件已加载: {plugin._name} v{plugin._version}")
            return {"success": True, "message": f"插件 {plugin_name} 已加载", "data": self._plugin_info(plugin)}
    ...
    async def _run_async_method(self, plugin: Plugin, method_name: str) -> None:
    ...
    def enable_plugin(self, plugin_name: str) -> dict[str, Any]:
        with self._lock:
            return self.enable_plugin_internal(plugin_name, set())

    def enable_plugin_internal(self, plugin_name: str, _enabling: set[str] | None = None) -> dict[str, Any]:
        ...
        for dep_name, _constraint in deps.items():
            ...
            if not dep or dep.status != PluginStatus.ENABLED:
                ...
                result = self.enable_plugin_internal(dep_name, _enabling)
    ...
    def _reload_single(self, plugin_name: str) -> None:
        logger.debug(f"[framework] _reload_single: {plugin_name} -> 卸载中...")
        # 通知前端清理该插件的残留
        self._emit_frontend("plugin:cleanup", {"name": plugin_name})
        self._unload_plugin_internal(plugin_name)
        logger.debug(f"[framework] _reload_single: {plugin_name} -> 加载中...")
        result = self._load_plugin_internal(plugin_name)
        if result["success"]:
            logger.info(f"重载插件 {plugin_name} 完成")
        else:
            logger.error(f"重载插件 {plugin_name} 加载失败: {result.get('message', '')}")
    ...
    def get_plugin_info_dict(self, plugin: Plugin) -> dict[str, Any]:
        # 获取插件信息字典，供 API 层使用
        return self._plugin_info(plugin)

    def _plugin_info(self, plugin: Plugin) -> dict[str, Any]:
        return {
            "name": plugin._name,
