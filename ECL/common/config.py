...
class ConfigManager:
    ...
    def _auto_complete_missing_config(self):
        ...
        for section, default_section_config in default_config.items():
            if section not in current_config:
                ...
                config_updated = True
            elif isinstance(default_section_config, dict) and isinstance(current_config[section], dict):
    ...
    def load(self):
        ...
        if env and self.config:
            for env_key, env_val in env.items():
                ...
                if section not in self.config[0] or key not in self.config[0][section]:
                    continue
                original_val = self.config[0][section][key]
                self.config[0][section][key] = convert_env_value(env_val, original_val)
    ...
    def _make_config_safe_for_json(self, obj):
    ...
    def update_locale_config(self, locale):
        if not self.config:
            self.config = self._get_default_config()
    ...
    def update_background_config(self, background_config):
        if not self.config:
            self.config = self._get_default_config()
        if "ui" not in self.config[0]:
            self.config[0]["ui"] = {}
    ...
    def _init_single_path(self, path):
        path_obj = Path(path)
    ...
    def check_game_paths_exist(self):
        game_config = self.config[0].get("game", {})
        paths = game_config.get("minecraft_paths", [self._get_default_minecraft_path()])
        results = []
        for p in paths:
    ...
    def get_game_config(self, auto_init=False):
    ...
    def _ensure_default_minecraft_path(self, paths):
        default_path = self._get_default_minecraft_path()
        found = False
    ...
    def update_game_config(self, game_config):
        if not self.config:
            self.config = self._get_default_config()
        ...
        if "minecraft_path" in updated_config and isinstance(updated_config["minecraft_path"], str):
            ...
            updated_config["minecraft_paths"] = [
                {"name": "默认路径", "path": self._get_default_minecraft_path(), "protected": True}
            ]
        ...
        for p in updated_config["minecraft_paths"]:
            path_str = p.get("path", "") if isinstance(p, dict) else p
            if path_str and path_str not in old_paths:
    ...
    def update_theme_config(self, theme_config):
        if not self.config:
            self.config = self._get_default_config()
        if "ui" not in self.config[0]:
            self.config[0]["ui"] = {}
        self.config[0]["ui"]["theme"] = {
    ...
    def update_download_config(self, download_config):
        if not self.config:
            self.config = self._get_default_config()
    ...
    def __repr__(self):
        return f"ConfigManager(config_path='{self._config_path!s}')"
