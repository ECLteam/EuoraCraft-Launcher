...
- 后续布局修正：主窗口样式选择框宽度设为 340px，弹出菜单挂在主窗口容器内；设置行只保留简短的重启与平台提示，详细平台差异留在本文档。纯自绘模式同时裁剪主窗口容器及其外层弹出层，避免元素越过自定义圆角。Windows 11 实机检查了系统边缘与纯自绘模式的菜单宽度、对齐和窗口边界；底部空间不足时菜单自动向上展开。
